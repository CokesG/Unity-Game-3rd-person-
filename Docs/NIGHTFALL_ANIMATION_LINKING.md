...
- Grounded full-quality procedural crouch-walk candidates now exist under `Assets/Art/Characters/NightfallVanguard/Exports/ProceduralCrouchWalk/`. They are generated by `Tools/Blender/create_nightfall_crouch_walk_procedural.py`, which grounds the mesh to the Blender grid per keyframe, lowers the stance, reduces leg crossing, and bakes temporary IK hand targets into a compact shooter-ready arm pocket before export.
- The linked sandbox `Crouch Walk` state starts from `Nightfall_FullQuality_CrouchWalk_Forward_Procedural` for visual review only. `NightfallAnimationSandboxDriver` auto-loads the forward/back/left/right procedural full-quality clips and uses an `AnimatorOverrideController` to cycle them with `Q` / `E` while `9 Crouched Walk` is active. The live `PlayerHumanoid.controller` remains quarantined for crouch-walk.
...
| Crouch Walk | sandbox review starts on `Nightfall_FullQuality_CrouchWalk_Forward_Procedural`; `Q` / `E` cycle forward/back/left/right procedural candidates; live quarantined |
...
9 Crouched Walk
0 Crouch -> Stand
Q/E Cycle procedural crouch-walk direction while 9 is active
```
...
`9 Crouched Walk` is the current sandbox review lane. It must stay low, grounded, readable from front and side, and consistent across forward/back/left/right before any live `SampleScene` promotion.
