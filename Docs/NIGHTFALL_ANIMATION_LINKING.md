...
- The linked sandbox keeps `Crouch Walk` quarantined by default. The controller state is mapped to the safe crouch hold, `9 Crouched Walk` routes to that same hold, and the HUD says the authored walk set is required. `NightfallAnimationSandboxDriver.enableProceduralCrouchWalkReview` can be enabled manually only for debugging the rejected forward/back/left/right procedural candidates with `Q` / `E`; do not treat that path as an approval lane.
...
| Crouch Walk | quarantined; sandbox `9` shows the safe crouch hold by default; procedural candidates require manual debug opt-in and are not approved |
...
9 Crouched Walk
0 Crouch -> Stand
Q/E Reserved for procedural crouch-walk debug only when `enableProceduralCrouchWalkReview` is manually enabled
```
...
`9 Crouched Walk` is currently a quarantine lane that shows the safe crouch hold. A future authored or motion-captured crouch-walk set must stay low, grounded, readable from front and side, and consistent across forward/back/left/right before any live `SampleScene` promotion.
