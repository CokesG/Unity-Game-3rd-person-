...
- Grounded full-quality procedural crouch-walk candidates live under `Assets/Art/Characters/NightfallVanguard/Exports/ProceduralCrouchWalk/`, but that generated set is rejected for hero use. The authored directional set lives under `Assets/Animations/NightfallVanguard/UserCrouchWalk/`.
...
- Current slide animation blend tuning: `slideEnterCrossFadeTime` is `0.08`, `slideExitToCrouchCrossFadeTime` is `0.22`, `slideExitToStandCrossFadeTime` is `0.30`, and `slideExitCrouchSettleDuration` is `0.35`. Slide-to-crouch must crossfade into the held crouch pose; do not use `Animator.Play` for that exit because it snaps to the held crouch frame. If the player stands right after a slide, the stand-up transition should use the softer post-slide blend.
- Current slide-to-crouch stability tuning: `slideGroundStickForce` is `18`, `slideExitSpeedCarry` is `1.1`, `slideStartGroundedStableTime` is `0.1`, `slideCrouchExitGroundStickTime` is `0.24`, `slideExitJumpLockTime` is `0.2`, `slideExitStandLockTime` is `0.30`, and `slideStepOffset` is `0`.
- Slide gameplay is active and `Base Layer.Slide` uses `User_Running_Slide` from `Assets/Animations/NightfallVanguard/UserSlide/User_Running_Slide.fbx` for live review. Active motor slide now forces the Slide state even if a stale Play Mode instance has `slideClipPromoted` off, because gameplay slide without the selected slide animation is harder to debug. Keep root motion disabled; the `CharacterController` owns slide distance and collision. Slide exit should resolve into the held crouch pose because crouch-walk remains quarantined. If the live pose reads upright/run-like again, fix or replace the clip rather than silently falling back to crouch hold.
- F1 animation telemetry shows the current movement state, `MovementX` / `MovementY`, whether crouch-walk is active, current Animator state path, crouch transition timer, post-slide crouch settle timer, and visual grounding offset.
...
For crouch-walk candidates:

- Use the authored files in `Assets/Animations/NightfallVanguard/UserCrouchWalk/` for current review.
- Run `Tools/TPS/Nightfall/Reimport Crouch Walk Candidates` in Unity after replacing or regenerating crouch-walk FBXs.
- Preview `9 Crouched Walk` in `Assets/Scenes/AnimationSandbox_Nightfall_Linked.unity`; use `Q` / `E` to cycle forward/back/left/right without editing the controller.
- Keep `forceCrouchWalkWhenMoving` off unless intentionally debugging fallback behavior. `crouchWalkClipPromoted` is the live approval switch for the authored set.
