...
- `Crouch Walk`: quarantined for live play; the current review candidates are grounded and compact, but not promoted to `SampleScene` yet.
...
- Grounded full-quality procedural crouch-walk candidates live under `Assets/Art/Characters/NightfallVanguard/Exports/ProceduralCrouchWalk/`. They are generated by `Tools/Blender/create_nightfall_crouch_walk_procedural.py`, which grounds the mesh to the Blender grid per keyframe, removes the earlier leg crossing, and uses temporary Blender IK hand targets so the arms stay in a compact two-hand shooter-ready pocket before export.
...
The procedural candidate set is not wired into `PlayerHumanoid.controller` yet. The linked sandbox `Crouch Walk` state starts from `Nightfall_FullQuality_CrouchWalk_Forward_Procedural` and `NightfallAnimationSandboxDriver` can auto-load the full-quality forward/back/left/right candidate clips; press `Q` / `E` while reviewing `9 Crouched Walk` to cycle the directional candidates through an `AnimatorOverrideController`. After the full directional set passes sandbox review, replace the live `Crouch Walk Directional` child motions and only then enable `crouchWalkClipPromoted`.
