...
- Slide gameplay is active and `PlayerAnimationController.slideClipPromoted` is enabled in `SampleScene`. `Base Layer.Slide` currently uses the stable held crouch pose from `User_Stand_To_Crouch` at normalized time `0.95` because Unity frame-audit renders showed `User_Running_Slide` reading as upright/run-like on the live rig instead of a clear low slide. Keep root motion disabled; the `CharacterController` owns slide distance and collision. Slide exit should resolve into the held crouch pose because crouch-walk remains quarantined.
