...
- Slide gameplay is active, but slide visuals are quarantined. `PlayerAnimationController.slideClipPromoted` stays off in `SampleScene`, so slide uses the stable crouch/held-pose visual path while the `CharacterController` owns slide distance and collision. The original baked `NightfallVanguard_Armature|slide_light` clip in `Assets/Art/Characters/NightfallVanguard/Exports/NightfallVanguard_Prototype_Animated.fbx` is rejected for the current full-quality rig because it produces a broken airborne/arms-up pose in live play. Use a fresh rig-compatible slide animation before enabling `slideClipPromoted`.
