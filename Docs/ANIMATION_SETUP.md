...
- Current slide animation blend tuning: `slideEnterCrossFadeTime` is `0.08`, and `slideExitToCrouchCrossFadeTime` is `0.22`. Slide-to-crouch must crossfade into the held crouch pose; do not use `Animator.Play` for that exit because it snaps to the held crouch frame.
- Current slide-to-crouch stability tuning: `slideGroundStickForce` is `18`, `slideExitSpeedCarry` is `1.1`, `slideStartGroundedStableTime` is `0.1`, `slideCrouchExitGroundStickTime` is `0.24`, `slideExitJumpLockTime` is `0.2`, `slideExitStandLockTime` is `0.18`, and `slideStepOffset` is `0`.
