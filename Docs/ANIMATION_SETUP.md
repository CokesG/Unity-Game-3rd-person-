...
- Slide is live-routed through `PlayerAnimationController.slideClipPromoted` to `Base Layer.Slide`, using the original baked `NightfallVanguard_Armature|slide_light` clip inside `Assets/Art/Characters/NightfallVanguard/Exports/NightfallVanguard_Prototype_Animated.fbx`. Keep root motion disabled; the `CharacterController` owns slide distance and collision. Slide exit should resolve into the held crouch pose because crouch-walk remains quarantined.
