...
- Slide gameplay is active and `PlayerAnimationController.slideClipPromoted` is enabled in `SampleScene`. `Base Layer.Slide` uses `User_Running_Slide` from `Assets/Animations/NightfallVanguard/UserSlide/User_Running_Slide.fbx`. Keep root motion disabled; the `CharacterController` owns slide distance and collision. Slide exit should resolve into the held crouch pose because crouch-walk remains quarantined.
