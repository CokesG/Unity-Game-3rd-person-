...
- Slide is live-routed through `PlayerAnimationController.slideClipPromoted` to `Base Layer.Slide`, using `Assets/Animations/NightfallVanguard/GLBClips/Nightfall_Slide.anim`. Keep root motion disabled; the `CharacterController` owns slide distance and collision.
