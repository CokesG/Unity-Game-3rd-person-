...
- Crouch gameplay is active. Stand-to-crouch, crouch idle, and crouch-to-stand are live-promoted. Crouch-walk is pulled from live play; moving crouched in `SampleScene` should hold the stable crouch pose until the authored directional set is re-audited. Grounded visual grounding uses foot/toe bones first and renderer bounds only as a fallback so the visual child can be corrected without moving the `Player` root or `CharacterController`.
- The authored crouch-walk directional set remains in the project for later review, but it is not part of the current live movement contract.
- Slide exit into crouch uses a ground-stick force and capped speed carry to avoid upward launch/pop when transitioning from slide to crouched locomotion.
