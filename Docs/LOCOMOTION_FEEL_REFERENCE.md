...
- Crouch gameplay is active. Stand-to-crouch, crouch idle, and crouch-to-stand are live-promoted. Crouch-walk is quarantined in live play and in the linked sandbox by default; `9 Crouched Walk` now shows the safe crouch hold because the procedural walk candidates still produce unacceptable leg/stance artifacts. Moving crouched in `SampleScene` should hold the reviewed crouch pose until an authored or motion-captured directional crouch-walk set passes review. Grounded visual grounding uses foot/toe bones first and renderer bounds only as a fallback so the visual child can be corrected without moving the `Player` root or `CharacterController`.
